...
const char* Clase = "Robot";
const char* ID = "Robot1";
...
bool sessionActive = false; // Indica si la sesión está activa
...
void webSocketEvent(WStype_t type, uint8_t *payload, size_t length)
{
    ...
    case WStype_DISCONNECTED:
        ...
        delay(500);
        sessionActive = false; // Desactiva la sesión al desconectarse
        break;

    case WStype_CONNECTED:
        Serial.println("Conectado al servidor WebSocket");
        delay(500); // deja un ligero retraso para que el servidor pueda responder
        String registro = String("clase=") + Clase + " ID=" + ID; // Mensaje de registro dinámico
    ...
    case WStype_TEXT:
        ...
        if (message.startsWith("session:active")) {
            sessionActive = true; // Activa la sesión si el servidor lo permite
            Serial.println("Sesión activada");
            webSocket.sendTXT("Sesión establecida para " + String(ID));
            ...
            if (command.equals("adelante")) {
                ...
                webSocket.sendTXT("Comando ejecutado: adelante");
                analogWrite(ENA, 100);
                analogWrite(ENB, 100);
            ...
            Serial.println("Sesión no activa. Comando ignorado.");
            webSocket.sendTXT("Error: Sesión no activa");
        }
        break;
    }
}
